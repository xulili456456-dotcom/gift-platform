...
export default function HomePage() {
  ...
  const freeMargin = Math.max(0, balance - holdings.reduce((s, h) => s + (h.cost || 0), 0));
  const marginPct = balance > 0 ? Math.round((freeMargin / balance) * 100) : 0;
  ...
  return (
    <div style={{ background: '#f2f2f7', minHeight: '100vh', maxWidth: 430, margin: '0 auto', paddingBottom: 80, position: 'relative', overflow: 'hidden' }}>
      ...
      <header style={{ background: 'linear-gradient(180deg,#0a0a0f,#1a1a24)', padding: '14px 16px 16px', color: '#fff', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          {/* Balance */}
          <div onClick={() => navigate('/mine/wallet')} style={{ flex: 1, background: 'rgba(255,255,255,.04)', border: '1px solid rgba(255,255,255,.06)', borderRadius: 12, padding: '10px 8px', cursor: 'pointer', position: 'relative', overflow: 'hidden', minWidth: 0, transition: 'all .2s' }}>
