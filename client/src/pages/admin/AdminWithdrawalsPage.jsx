...
export default function AdminWithdrawalsPage() {
  ...
  const handleUpdate = async (id, status) => {
    try {
      ...
      toast.success(status === 'completed' ? '已通过' : '已拒绝');
      loadData();
  ...
  return (
    <div className="p-4 space-y-4 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold text-text-primary">提现管理</h1>
        <span className="text-xs text-text-muted bg-gray-100 px-2 py-0.5 rounded-full">{total}</span>
      </div>

      <div className="flex gap-2">
        {[{ v: '', l: '全部' }, { v: 'pending', l: '待处理' }, { v: 'completed', l: '已完成' }, { v: 'rejected', l: '已拒绝' }].map(f => (
            ...
            <div key={w.id} className="bg-white rounded-2xl p-4 shadow-sm border">
              <div className="flex justify-between mb-2">
                ...
                <div className="text-right">
                  ...
                  }`}>{w.status === 'pending' ? '待处理' : w.status === 'completed' ? '已完成' : '已拒绝'}</span>
                </div>
              </div>
              <div className="text-xs text-text-muted mb-3">
                <p>网络: {w.network?.toUpperCase()} · 地址: {w.wallet_address?.slice(0, 10)}...{w.wallet_address?.slice(-6)}</p>
                <p>申请时间: {w.created_at?.slice(0, 16)}</p>
              </div>
              {w.status === 'pending' && (
                <div className="flex gap-2">
                  <button onClick={() => handleUpdate(w.id, 'completed')} className="flex-1 py-2 bg-green-500 text-white rounded-xl text-sm font-semibold">通过</button>
                  <button onClick={() => handleUpdate(w.id, 'rejected')} className="flex-1 py-2 bg-red-500 text-white rounded-xl text-sm font-semibold">拒绝</button>
                </div>
              )}
            </div>
          ))}
