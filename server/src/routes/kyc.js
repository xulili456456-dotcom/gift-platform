...
router.get('/', authMiddleware, (req, res) => {
  const kyc = get('SELECT * FROM kyc_submissions WHERE user_id = ?', [req.user.id]);
  res.json(kyc || null);
});
...
router.post('/', authMiddleware, (req, res) => {
  ...
  const existing = get('SELECT id FROM kyc_submissions WHERE user_id = ?', [req.user.id]);
...
router.put('/admin/:id', authMiddleware, adminMiddleware, (req, res) => {
  ...
  run('UPDATE kyc_submissions SET status = ?, admin_note = ?, reviewed_at = datetime(\'now\') WHERE id = ?',
